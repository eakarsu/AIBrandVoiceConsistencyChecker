...
export default function NoRealTimeVoiceScoringOnLiveDraftsStreaminPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-real-time-voice-scoring-on-live-drafts-streamin/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No real-time voice scoring on live drafts (streaming)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-real-time-voice-scoring-on-live-drafts-streamin/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
