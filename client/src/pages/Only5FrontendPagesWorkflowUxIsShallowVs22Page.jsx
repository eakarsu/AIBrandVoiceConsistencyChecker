...
export default function Only5FrontendPagesWorkflowUxIsShallowVs22Page() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-only-5-frontend-pages-workflow-ux-is-shallow-vs-22/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 5 frontend pages — workflow UX is shallow vs. 22 backend routes</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-5-frontend-pages-workflow-ux-is-shallow-vs-22/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
