...
router.get('/', auth, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM content_history ORDER BY created_at DESC');
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [title, content, channel, author, version || '1.0', brand_profile_id, change_type || 'created']
    );
