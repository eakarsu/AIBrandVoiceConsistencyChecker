...
router.get('/', auth, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM content_templates ORDER BY created_at DESC');
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [name, category, channel, template_content, JSON.stringify(variables), brand_profile_id]
    );
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [name, category, channel, template_content, JSON.stringify(variables), brand_profile_id, req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Not found' });
    res.json(result.rows[0]);
  } catch (err) {
