...
const ALLOWED_EVENTS = [
  'content.analyzed', 'content.flagged_off_brand', 'content.approved',
  'voice.drift_detected', 'competitor.update_detected',
  'audit.report_generated', 'guideline.updated'
];
...
async function ensureTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS webhooks (
      ...
      user_id INTEGER,
      url TEXT NOT NULL,
      events TEXT[] NOT NULL DEFAULT ARRAY['content.analyzed']::text[],
      secret VARCHAR(255),
...
router.post('/', async (req, res) => {
  try {
    ...
    const incoming = Array.isArray(events) && events.length > 0 ? events : ['content.analyzed'];
...
router.post('/:id/test', async (req, res) => {
  try {
    ...
    const payload = {
      ...
      data: { message: 'Test webhook from AIBrandVoiceConsistencyChecker' }
    };
